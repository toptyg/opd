...
define("RadarChart", ["d3"], function(d3) {
    return function (id, data, options) {
	
/*	var LegendOptions = ['Lecture','Practice'];*/
...
*/
                ...
                while ((word = words.pop())) {
                    ...
                    if (tspan.node().getComputedTextLength() * (8/11) > width) {
                        line.pop();
                        tspan.text(line.join(" "));
                        line = [word];
                        tspan = text.append("tspan").attr("x", x).attr("y", y).attr(
